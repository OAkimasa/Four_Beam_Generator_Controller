...
# TTM_solve result viewer
def result_viewer(param_list):
    print(TTM1_params, "TTM1")
    TTM1_nV = [param_list[0], param_list[1], param_list[2], param_list[3]]
    TTM2_nV = [param_list[4], param_list[5], param_list[6], param_list[7]]
    ...
    def FBM_raytrace():
        ...
        for i in range(4):
            T_y = (90.6 - pos_FBM[i][1])/dir_FBM[i][1]
            T_z = (0 - pos_FBM[i][2])/dir_FBM[i][2]
            rayi_pupill_x = pos_FBM[i][0] + dir_FBM[i][0]*((T_y+T_z)/2)
            print(rayi_pupill_x)
            print((T_y+T_z)/2)
    ...
    def Evaluation_Plane_raytrace():
        ...
        print("ray1(blue):", pos_Evaluation_Plane[0], dir_Evaluation_Plane[0]/np.linalg.norm(dir_Evaluation_Plane[0]), np.round(deg_list[0]*3600, 6), "arcsec_result")
        print("ray2(green):", pos_Evaluation_Plane[1], dir_Evaluation_Plane[1]/np.linalg.norm(dir_Evaluation_Plane[1]), np.round(deg_list[1]*3600, 6), "arcsec_result")
        print("ray3(red):", pos_Evaluation_Plane[2], dir_Evaluation_Plane[2]/np.linalg.norm(dir_Evaluation_Plane[2]), np.round(deg_list[2]*3600, 6), "arcsec_result")
        print("ray4(orange):", pos_Evaluation_Plane[3], dir_Evaluation_Plane[3]/np.linalg.norm(dir_Evaluation_Plane[3]), np.round(deg_list[3]*3600, 6), "arcsec_result\n")

        print("ray1-ray4:", np.round(np.degrees(np.arccos(np.dot(last_dir[0], last_dir[3])))*3600, 6), "arcsec_result")
        print("ray2-ray3:", np.round(np.degrees(np.arccos(np.dot(last_dir[1], last_dir[2])))*3600, 6), "arcsec_result\n")
    ...
    print(last_dir, "last_dir_result")
    
    target_index = 2  # 0:10arcsec, 1:20arcsec, 2:30arcsec, 3:40arcsec
