...
def mirror_reflection():
    ...
    # 光線追跡
    VF.plotLineBlack(ax, [-200,90.6,0], [1955.5,90.6,0])  # 仮
    VF.plotLineBlack(ax, [-100,0,0], [400,0,0])  # 仮
    ...
    def BS_raytrace():
        ...
        front_pos_BS1 = s_pos + s_dir*VF.calcT_plane(s_pos, s_dir, pos_BS1_f, nV_BS1_f)
        front_dir_BS1 = VF.calcReflectionV(s_dir, nV_BS1_f)
        VF.plotLineBlue(ax, s_pos, front_pos_BS1)
        ...
        back_pos_BS1 = front_pos_BS1 + glass_dir_BS1*VF.calcT_plane(front_pos_BS1, glass_dir_BS1, pos_BS1_b, nV_BS1_b)
        back_dir_BS1 = VF.calcRefractionV(glass_dir_BS1, nV_BS1_b, N_Fused_Silica, N_air)
        VF.plotLineGreen(ax, front_pos_BS1, back_pos_BS1)

        # ----------------BS2----------------
        pos_BS2_f = BS_params[2][0]
        nV_BS2_f = BS_params[2][1]
        nV_BS2_f = nV_BS2_f / np.linalg.norm(nV_BS2_f)
        front_pos_BS2 = back_pos_BS1 + back_dir_BS1*VF.calcT_plane(back_pos_BS1, back_dir_BS1, pos_BS2_f, nV_BS2_f)
        front_dir_BS2 = VF.calcReflectionV(back_dir_BS1, nV_BS2_f)
        VF.plotLineGreen(ax, back_pos_BS1, front_pos_BS2)
        ...
        back_pos_BS2 = front_pos_BS2 + glass_dir_BS2*VF.calcT_plane(front_pos_BS2, glass_dir_BS2, pos_BS2_b, nV_BS2_b)
        back_dir_BS2 = VF.calcRefractionV(glass_dir_BS2, nV_BS2_b, N_Fused_Silica, N_air)
        VF.plotLineRed(ax, front_pos_BS2, back_pos_BS2)

        # ----------------BS3----------------
        pos_BS3_f = BS_params[4][0]
        nV_BS3_f = BS_params[4][1]
        nV_BS3_f = nV_BS3_f / np.linalg.norm(nV_BS3_f)
        front_pos_BS3 = back_pos_BS2 + back_dir_BS2*VF.calcT_plane(back_pos_BS2, back_dir_BS2, pos_BS3_f, nV_BS3_f)
        front_dir_BS3 = VF.calcReflectionV(back_dir_BS2, nV_BS3_f)
        VF.plotLineRed(ax, back_pos_BS2, front_pos_BS3)
        ...
        back_pos_BS3 = front_pos_BS3 + glass_dir_BS3*VF.calcT_plane(front_pos_BS3, glass_dir_BS3, pos_BS3_b, nV_BS3_b)
        back_dir_BS3 = VF.calcRefractionV(glass_dir_BS3, nV_BS3_b, N_Fused_Silica, N_air)
        VF.plotLineOrange(ax, front_pos_BS3, back_pos_BS3)

        # ----------------BS4----------------
        pos_BS4_f = BS_params[6][0]
        nV_BS4_f = BS_params[6][1]
        nV_BS4_f = nV_BS4_f / np.linalg.norm(nV_BS4_f)
        front_pos_BS4 = back_pos_BS3 + back_dir_BS3*VF.calcT_plane(back_pos_BS3, back_dir_BS3, pos_BS4_f, nV_BS4_f)
        front_dir_BS4 = VF.calcReflectionV(back_dir_BS3, nV_BS4_f)
        VF.plotLineOrange(ax, back_pos_BS3, front_pos_BS4)
    ...
    def TTM1_raytrace():
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], TTM1_params[i][0], nV_TTM1)
            dir = VF.calcReflectionV(last_dir[i], nV_TTM1)
            pos_TTM1.append(pos)
            dir_TTM1.append(dir)

        VF.plotLineBlue(ax, last_pos[0], pos_TTM1[0])
        VF.plotLineGreen(ax, last_pos[1], pos_TTM1[1])
        VF.plotLineRed(ax, last_pos[2], pos_TTM1[2])
        VF.plotLineOrange(ax, last_pos[3], pos_TTM1[3])
    ...
    def TTM2_raytrace():
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], TTM2_params[i][0], nV_TTM2)
            dir = VF.calcReflectionV(last_dir[i], nV_TTM2)
            pos_TTM2.append(pos)
            dir_TTM2.append(dir)

        VF.plotLineBlue(ax, last_pos[0], pos_TTM2[0])
        VF.plotLineGreen(ax, last_pos[1], pos_TTM2[1])
        VF.plotLineRed(ax, last_pos[2], pos_TTM2[2])
        VF.plotLineOrange(ax, last_pos[3], pos_TTM2[3])
    ...
    def PRISM_front_raytrace():
        ...
        for i in range(4):
            ...
            #print(nV_PRISM)
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], PRISM_front_params[i][0], nV_PRISM_front)
        ...
        VF.plotLineBlue(ax, last_pos[0], pos_PRISM_front[0])
        VF.plotLineGreen(ax, last_pos[1], pos_PRISM_front[1])
        VF.plotLineRed(ax, last_pos[2], pos_PRISM_front[2])
        VF.plotLineOrange(ax, last_pos[3], pos_PRISM_front[3])
    ...
    def PRISM_back_raytrace():
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], PRISM_back_params[i][0], nV_PRISM_back)
            dir = VF.calcRefractionV(last_dir[i], nV_PRISM_back, N_Fused_Silica, N_air)
            pos_PRISM_back.append(pos)
            dir_PRISM_back.append(dir)

        VF.plotLineBlue(ax, last_pos[0], pos_PRISM_back[0])
        VF.plotLineGreen(ax, last_pos[1], pos_PRISM_back[1])
        VF.plotLineRed(ax, last_pos[2], pos_PRISM_back[2])
        VF.plotLineOrange(ax, last_pos[3], pos_PRISM_back[3])
    ...
    def FBM_raytrace():
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], FBM_params[0][0], nV_FBM)
            dir = VF.calcReflectionV(last_dir[i], nV_FBM)
            pos_FBM.append(pos)
            dir_FBM.append(dir)

        VF.plotLineBlue(ax, last_pos[0], pos_FBM[0])
        VF.plotLineGreen(ax, last_pos[1], pos_FBM[1])
        VF.plotLineRed(ax, last_pos[2], pos_FBM[2])
        VF.plotLineOrange(ax, last_pos[3], pos_FBM[3])
    ...
    def M5_raytrace():
        ...
        for i in range(4):
            nV_M5 = M5_params[0][1]
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], M5_params[0][0], nV_M5)
            dir = VF.calcReflectionV(last_dir[i], nV_M5)
            pos_M5.append(pos)
            dir_M5.append(dir)

        VF.plotLineBlue(ax, last_pos[0], pos_M5[0])
        VF.plotLineGreen(ax, last_pos[1], pos_M5[1])
        VF.plotLineRed(ax, last_pos[2], pos_M5[2])
        VF.plotLineOrange(ax, last_pos[3], pos_M5[3])
    ...
    def DM1234_raytrace():
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], DM1234_params[0][0], nV_DM1234)
            dir = VF.calcReflectionV(last_dir[i], nV_DM1234)
            pos_DM1234.append(pos)
            dir_DM1234.append(dir)

        VF.plotLineBlue(ax, last_pos[0], pos_DM1234[0])
        VF.plotLineGreen(ax, last_pos[1], pos_DM1234[1])
        VF.plotLineRed(ax, last_pos[2], pos_DM1234[2])
        VF.plotLineOrange(ax, last_pos[3], pos_DM1234[3])
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], DM1234_params[1][0], nV_DM1234)
            dir = VF.calcReflectionV(last_dir[i], nV_DM1234)
            pos_DM1234.append(pos)
            dir_DM1234.append(dir)
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], DM1234_params[2][0], nV_DM1234)
            dir = VF.calcReflectionV(last_dir[i], nV_DM1234)
            pos_DM1234.append(pos)
            dir_DM1234.append(dir)
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], DM1234_params[3][0], nV_DM1234)
            dir = VF.calcReflectionV(last_dir[i], nV_DM1234)
            pos_DM1234.append(pos)
            dir_DM1234.append(dir)
    ...
    def IMR_raytrace():
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], IMR_params[0][0], nV_IMR)
            dir = VF.calcRefractionV(last_dir[i], nV_IMR, N_air, N_Fused_Silica)
            pos_IMR.append(pos)
            dir_IMR.append(dir)

        VF.plotLineBlue(ax, last_pos[0], pos_IMR[0])
        VF.plotLineGreen(ax, last_pos[1], pos_IMR[1])
        VF.plotLineRed(ax, last_pos[2], pos_IMR[2])
        VF.plotLineOrange(ax, last_pos[3], pos_IMR[3])
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], IMR_params[1][0], nV_IMR)
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], IMR_params[2][0], nV_IMR)
    ...
    def DM56_raytrace():
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], DM56_params[0][0], nV_DM56)
            dir = VF.calcReflectionV(last_dir[i], nV_DM56)
            pos_DM56.append(pos)
            dir_DM56.append(dir)

        VF.plotLineBlue(ax, last_pos[0], pos_DM56[0])
        VF.plotLineGreen(ax, last_pos[1], pos_DM56[1])
        VF.plotLineRed(ax, last_pos[2], pos_DM56[2])
        VF.plotLineOrange(ax, last_pos[3], pos_DM56[3])
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], DM56_params[1][0], nV_DM56)
            dir = VF.calcReflectionV(last_dir[i], nV_DM56)
            pos_DM56.append(pos)
            dir_DM56.append(dir)
    ...
    def BE_G123_raytrace():
        ...
        for i in range(4):
            pos = last_pos[i] + last_dir[i]*VF.calcT_sphere(last_pos[i], last_dir[i], BE_G123_params[0][0], lens_R_G1f)
            nV_BE_G123 = pos + np.array([0, 0, lens_R_G1f]) - BE_G123_params[0][0]
        ...
        VF.plotLineBlue(ax, last_pos[0], pos_BE_G123[0])
        VF.plotLineGreen(ax, last_pos[1], pos_BE_G123[1])
        VF.plotLineRed(ax, last_pos[2], pos_BE_G123[2])
        VF.plotLineOrange(ax, last_pos[3], pos_BE_G123[3])
        ...
        for i in range(4):
            pos = last_pos[i] + last_dir[i]*VF.calcT_sphere(last_pos[i], last_dir[i], BE_G123_params[1][0], lens_R_G1b)
            nV_BE_G123 = pos + np.array([0, 0, lens_R_G1b]) - BE_G123_params[1][0]
        ...
        for i in range(4):
            pos = last_pos[i] + last_dir[i]*VF.calcT_sphere(last_pos[i], last_dir[i], BE_G123_params[2][0], lens_R_G2f)
            nV_BE_G123 = pos + np.array([0, 0, lens_R_G2f]) - BE_G123_params[2][0]
            dir = VF.calcRefractionV(last_dir[i], -nV_BE_G123, N_air, N_Fused_Silica)
            #ax.quiver(pos[0], pos[1], pos[2], nV_BE_G123[0], nV_BE_G123[1], nV_BE_G123[2], color='blue', length=0.1)
            #ax.quiver(pos[0], pos[1], pos[2], -nV_BE_G123[0], -nV_BE_G123[1], -nV_BE_G123[2], color='blue', length=0.1)
            pos_BE_G123.append(pos)
        ...
        for i in range(4):
            pos = last_pos[i] + last_dir[i]*VF.calcT_sphere(last_pos[i], last_dir[i], BE_G123_params[3][0], lens_R_G2b)
            nV_BE_G123 = pos + np.array([0, 0, lens_R_G2b]) - BE_G123_params[3][0]
            dir = VF.calcRefractionV(last_dir[i], -nV_BE_G123, N_Fused_Silica, N_air)
            #ax.quiver(pos[0], pos[1], pos[2], nV_BE_G123[0], nV_BE_G123[1], nV_BE_G123[2], color='blue', length=0.1)
            #ax.quiver(pos[0], pos[1], pos[2], -nV_BE_G123[0], -nV_BE_G123[1], -nV_BE_G123[2], color='blue', length=0.1)
            pos_BE_G123.append(pos)
        ...
        for i in range(4):
            pos = last_pos[i] + last_dir[i]*VF.calcT_sphere(last_pos[i], last_dir[i], BE_G123_params[4][0], lens_R_G3f)
            nV_BE_G123 = pos + np.array([0, 0, lens_R_G3f]) - BE_G123_params[4][0]
            dir = VF.calcRefractionV(last_dir[i], -nV_BE_G123, N_air, N_Fused_Silica)
            #ax.quiver(pos[0], pos[1], pos[2], nV_BE_G123[0], nV_BE_G123[1], nV_BE_G123[2], color='blue', length=0.1)
            #ax.quiver(pos[0], pos[1], pos[2], -nV_BE_G123[0], -nV_BE_G123[1], -nV_BE_G123[2], color='blue', length=0.1)
            pos_BE_G123.append(pos)
        ...
        for i in range(4):
            pos = last_pos[i] + last_dir[i]*VF.calcT_sphere(last_pos[i], last_dir[i], BE_G123_params[5][0], lens_R_G3b)
            nV_BE_G123 = pos + np.array([0, 0, lens_R_G3b]) - BE_G123_params[5][0]
            dir = VF.calcRefractionV(last_dir[i], -nV_BE_G123, N_Fused_Silica, N_air)
            #ax.quiver(pos[0], pos[1], pos[2], nV_BE_G123[0], nV_BE_G123[1], nV_BE_G123[2], color='blue', length=0.1)
            #ax.quiver(pos[0], pos[1], pos[2], -nV_BE_G123[0], -nV_BE_G123[1], -nV_BE_G123[2], color='blue', length=0.1)
            pos_BE_G123.append(pos)
    ...
    def M678_raytrace():
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], M678_params[0][0], nV_M678)
            dir = VF.calcReflectionV(last_dir[i], nV_M678)
            pos_M678.append(pos)
            dir_M678.append(dir)

        VF.plotLineBlue(ax, last_pos[0], pos_M678[0])
        VF.plotLineGreen(ax, last_pos[1], pos_M678[1])
        VF.plotLineRed(ax, last_pos[2], pos_M678[2])
        VF.plotLineOrange(ax, last_pos[3], pos_M678[3])
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], M678_params[1][0], nV_M678)
            dir = VF.calcReflectionV(last_dir[i], nV_M678)
            pos_M678.append(pos)
            dir_M678.append(dir)
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], M678_params[2][0], nV_M678)
            dir = VF.calcReflectionV(last_dir[i], nV_M678)
            pos_M678.append(pos)
            dir_M678.append(dir)
    ...
    def LLT_Window_raytrace():
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], LLT_Window_params[0][0], nV_LLT_Window)
            dir = VF.calcRefractionV(last_dir[i], nV_LLT_Window, N_air, N_BSL7)
            pos_LLT_Window.append(pos)
            dir_LLT_Window.append(dir)

        VF.plotLineBlue(ax, last_pos[0], pos_LLT_Window[0])
        VF.plotLineGreen(ax, last_pos[1], pos_LLT_Window[1])
        VF.plotLineRed(ax, last_pos[2], pos_LLT_Window[2])
        VF.plotLineOrange(ax, last_pos[3], pos_LLT_Window[3])
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], LLT_Window_params[1][0], nV_LLT_Window)
    ...
    def LLT_M321_raytrace():
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], LLT_M321_params[0][0], nV_LLT_M321)
            dir = VF.calcReflectionV(last_dir[i], nV_LLT_M321)
            pos_LLT_M321.append(pos)
            dir_LLT_M321.append(dir)

        VF.plotLineBlue(ax, last_pos[0], pos_LLT_M321[0])
        VF.plotLineGreen(ax, last_pos[1], pos_LLT_M321[1])
        VF.plotLineRed(ax, last_pos[2], pos_LLT_M321[2])
        VF.plotLineOrange(ax, last_pos[3], pos_LLT_M321[3])
        ...
        for i in range(4):
            pos = last_pos[i] + last_dir[i]*VF.calcT_parabola(last_pos[i], last_dir[i], LLT_M321_params[1][0], parabola_R_LLT_M2)
            nV_LLT_M321 = VF.calcNormal_parabola(pos, LLT_M321_params[1][0], parabola_R_LLT_M2)
        ...
        for i in range(4):
            pos = last_pos[i] + last_dir[i]*VF.calcT_parabola(last_pos[i], last_dir[i], LLT_M321_params[2][0], parabola_R_LLT_M1)
            nV_LLT_M321 = VF.calcNormal_parabola(pos, LLT_M321_params[2][0], parabola_R_LLT_M1)
            #ax.quiver(pos[0], pos[1], pos[2], nV_LLT_M321[0], nV_LLT_M321[1], nV_LLT_M321[2], color='r', length=10)
            dir = VF.calcReflectionV(last_dir[i], nV_LLT_M321)
            pos_LLT_M321.append(pos)
            dir_LLT_M321.append(dir)
    ...
    def LLT_Exit_Window_raytrace():
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], LLT_Exit_Window_params[0][0], nV_LLT_Exit_Window)
        ...
        VF.plotLineBlue(ax, last_pos[0], pos_LLT_Exit_Window[0])
        VF.plotLineGreen(ax, last_pos[1], pos_LLT_Exit_Window[1])
        VF.plotLineRed(ax, last_pos[2], pos_LLT_Exit_Window[2])
        VF.plotLineOrange(ax, last_pos[3], pos_LLT_Exit_Window[3])
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], LLT_Exit_Window_params[1][0], nV_LLT_Exit_Window)
    ...
    def Evaluation_Plane_raytrace():
        ...
        for i in range(4):
            ...
            pos = last_pos[i] + last_dir[i]*VF.calcT_plane(last_pos[i], last_dir[i], Evaluation_Plane_params[0][0], nV_Evaluation_Plane)
            dir = last_dir[i]
            pos_Evaluation_Plane.append(pos)
            dir_Evaluation_Plane.append(dir)

        VF.plotLineBlue(ax, last_pos[0], pos_Evaluation_Plane[0])
        VF.plotLineGreen(ax, last_pos[1], pos_Evaluation_Plane[1])
        VF.plotLineRed(ax, last_pos[2], pos_Evaluation_Plane[2])
        VF.plotLineOrange(ax, last_pos[3], pos_Evaluation_Plane[3])
    ...
    VF.plotLineBlue(ax, last_pos[0], end_pos[0])
    VF.plotLineGreen(ax, last_pos[1], end_pos[1])
    VF.plotLineRed(ax, last_pos[2], end_pos[2])
    VF.plotLineOrange(ax, last_pos[3], end_pos[3])
    VF.plotLineBlack(ax, [60.000, -250.400, -370.000], [60.000, 2000, -370.000])
