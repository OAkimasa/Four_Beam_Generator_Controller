...
# ミラーの光線追跡を行う関数
def mirror_reflection(mode_flag):
    # インスタンス生成
    VF = VectorFunctions()
    ...
    pointsY, pointsZ = np.meshgrid(
                    ...
                    np.arange(-start_radius, start_radius+0.01, d))
    pointsX = np.array([0]*size)
    pointsY = pointsY.reshape(size)
    pointsZ = pointsZ.reshape(size)
    start_pos_spotdiagram = VF.makePoints(pointsX, pointsY, pointsZ, size, 3)
    print(start_pos_spotdiagram)
    #s_dir_spotdiagram = np.array([1.0, 0.0, 0.0]*len(s_pos_spotdiagram))
    
    # chiefray
    def plot_chiefray(s_pos_in, s_dir_in):
        # BS
        def BS_raytrace():
            # ----------------BS1----------------
            ...
            if s_pos_in[1]**2 + s_pos_in[2]**2 > 5.9**2:
                s_pos = s_pos_in
        ...
        def PRISM_front_raytrace():
            ...
            if pos_PRISM_front[0][0] < -125 or pos_PRISM_front[0][2] < 0:
                dir_PRISM_front[0] = np.array([0, 0, 0])
            elif -125 < pos_PRISM_front[1][0] or pos_PRISM_front[1][2] < 0:
                dir_PRISM_front[1] = np.array([0, 0, 0])
            elif pos_PRISM_front[2][0] < -125 or 0 < pos_PRISM_front[2][2]:
                dir_PRISM_front[2] = np.array([0, 0, 0])
            elif -125 < pos_PRISM_front[3][0] or 0 < pos_PRISM_front[3][2]:
                dir_PRISM_front[3] = np.array([0, 0, 0])
        ...
        def PRISM_back_raytrace():
            ...
            if pos_PRISM_back[0][0] < -125 or pos_PRISM_back[0][2] < 0:
                dir_PRISM_back[0] = np.array([0, 0, 0])
            elif -125 < pos_PRISM_back[1][0] or pos_PRISM_back[1][2] < 0:
                dir_PRISM_back[1] = np.array([0, 0, 0])
            elif pos_PRISM_back[2][0] < -125 or 0 < pos_PRISM_back[2][2]:
                dir_PRISM_back[2] = np.array([0, 0, 0])
            elif -125 < pos_PRISM_back[3][0] or 0 < pos_PRISM_back[3][2]:
                dir_PRISM_back[3] = np.array([0, 0, 0])
        ...
        def Evaluation_Plane_raytrace():
            ...
            for i in range(4):
                ...
                #rad_list.append(np.arccos(np.sqrt(1-last_dir[i][0]**2-last_dir[i][2]**2)))
            deg_list = np.degrees(rad_list)*3600  # deg to arcsec
            print(deg_list, "case1")

            rad_list = []
            for i in range(4):
                #rad_list.append(np.arccos(-np.dot(last_dir[i], Evaluation_Plane_params[0][1])))
                rad_list.append(np.arccos(np.sqrt(1-last_dir[i][0]**2-last_dir[i][2]**2)))
            deg_list = np.degrees(rad_list)*3600
            print(deg_list, "case2")

            print("\n(Evaluation_Plane)")
            print("IMR_rot_deg:", IMR_rot_deg, "deg\n")
            print("ray1(blue):", pos_Evaluation_Plane[0], dir_Evaluation_Plane[0]/np.linalg.norm(dir_Evaluation_Plane[0]), np.round(deg_list[0], 6), "arcsec")
            print("ray2(green):", pos_Evaluation_Plane[1], dir_Evaluation_Plane[1]/np.linalg.norm(dir_Evaluation_Plane[1]), np.round(deg_list[1], 6), "arcsec")
            print("ray3(red):", pos_Evaluation_Plane[2], dir_Evaluation_Plane[2]/np.linalg.norm(dir_Evaluation_Plane[2]), np.round(deg_list[2], 6), "arcsec")
            print("ray4(orange):", pos_Evaluation_Plane[3], dir_Evaluation_Plane[3]/np.linalg.norm(dir_Evaluation_Plane[3]), np.round(deg_list[3], 6), "arcsec\n")
    ...
    # many rays
    def plot_rays():
        for i in start_pos_spotdiagram:
            plot_chiefray(i, np.array([1.0, 0.0, 0.0]))

    # spot diagram
    def spot_diagram():
        # chief ray----------------------------------------------------------------------------------------------------------------------------
        s_pos_in = np.array([0, 0, 0])
        s_dir_in = np.array([1.0, 0.0, 0.0])
        # BS
        def BS_raytrace():
            # ----------------BS1----------------
            pos_BS1_f = BS_params[0][0]
        ...
        def PRISM_front_raytrace():
            ...
            VF.plotLineRed(last_pos[2], pos_PRISM_front[2])
            VF.plotLineOrange(last_pos[3], pos_PRISM_front[3])"""
            print(dir_PRISM_front, "dir_PRISM_front")
            return pos_PRISM_front, dir_PRISM_front
        ...
        def Evaluation_Plane_raytrace():
            ...
            plt.scatter(pos_Evaluation_Plane[0][0], pos_Evaluation_Plane[0][2], color='k', s=30, marker="x")
            plt.scatter(pos_Evaluation_Plane[1][0], pos_Evaluation_Plane[1][2], color='k', s=30, marker="x")
            plt.scatter(pos_Evaluation_Plane[2][0], pos_Evaluation_Plane[2][2], color='k', s=30, marker="x")
            plt.scatter(pos_Evaluation_Plane[3][0], pos_Evaluation_Plane[3][2], color='k', s=30, marker="x")
        ...
        theta = np.linspace(0, 2*np.pi, 100)
        radius = 253
        x = radius*np.cos(theta) + 60
        z = radius*np.sin(theta) - 370
        ax.plot(x, z, color='black', linewidth=0.5)
        ...
        for i in start_pos_spotdiagram:
            ...
            def Evaluation_Plane_raytrace():
                ...
                plt.scatter(pos_Evaluation_Plane[0][0], pos_Evaluation_Plane[0][2], color='blue', s=7)
                plt.scatter(pos_Evaluation_Plane[1][0], pos_Evaluation_Plane[1][2], color='green', s=7)
                plt.scatter(pos_Evaluation_Plane[2][0], pos_Evaluation_Plane[2][2], color='red', s=7)
                plt.scatter(pos_Evaluation_Plane[3][0], pos_Evaluation_Plane[3][2], color='orange', s=7)
    ...
    if mode_flag==0:
        ...
        plot_chiefray(np.array([0.0, 0.0, 0.0]), np.array([1.0, 0.0, 0.0]))
        # 描画範囲
        LX = 300
        LY = 300
        LZ = 300
...
if __name__ == "__main__":
    ...
    LX = LY = LZ = 25
    ax.set_xlim(-LX-120, LX-120)
    ax.set_ylim(-LY+166, LY+166)
    ax.set_zlim(-LZ-0, LZ-0)          # prism: -120, 166, 0
    ...
    ax.set_ylim(-LY+150, LY+150)
    ax.set_zlim(-LZ-0, LZ-0)          # BE: 430, 90.6, -400
    ...
    mirror_reflection(mode_flag=1)
        # 描画範囲
    LX = LY = LZ = 100
    ax.set_xlim(-LX+430, LX+430)
    ax.set_ylim(-LY+280.6, LY+280.6)
    ax.set_zlim(-LZ-507.1, LZ-507.1)            # IMR: 430, 280.6, -507.1
    ax.set_xlabel('x [mm]')
    ax.set_ylabel('y [mm]')
    ax.set_zlabel('z [mm]')
    ax.view_init(elev=22, azim=45)"""
    ...
    mirror_reflection(mode_flag=2)
    LX = LY = 300
    ax.set_xlim(-LX+60, LX+60)
    ax.set_ylim(-LY-370, LY-370)     # evaluation plane: 60, 90.6, -370
    ax.set_xlabel('x [mm]')
    ax.set_ylabel('z [mm]')
    ax.set_aspect(1)
