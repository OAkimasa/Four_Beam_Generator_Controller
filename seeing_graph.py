...
w0 = 0.112  # m
# multiplyt by sqrt(2) to adjust the waist size in the fresnel propagator
w0_pix = w0 * math.sqrt(2.0) / pixel_scale

gauss = np.exp(-2*R**2/w0_pix**2)

# 2D Gaussian fitting
initial_guess = (1.0, 2.0*N_pix, 2.0*N_pix, 0.1*w0/pixel_scale, 1.0, 0)
...
def make_beam_image_10arcsec():
    ...
    r_2 = np.sqrt((mask_X-N_pix/2+0.0685*N_pix)**2 +
                  (mask_Y-N_pix/2+0.0666*N_pix)**2)
    ...
    r_3 = np.sqrt((mask_X-N_pix/2+0.0685*N_pix)**2 +
                  (mask_Y-N_pix/2+0.0666*N_pix)**2)
...
def make_beam_image_20arcsec():
    ...
    r_2 = np.sqrt((mask_X-N_pix/2+0.0628*N_pix)**2 +
                  (mask_Y-N_pix/2+0.0642*N_pix)**2)
    mask2_radius = 0.247  # m
    mask2 = np.copy(r_2)*0.0
    idx2_1 = np.where(r_2 >= mask2_radius / (1./128.))
    idx2_2 = np.where(r_2 < mask2_radius / (1./128.))
    ...
    r_3 = np.sqrt((mask_X-N_pix/2+0.0628*N_pix)**2 +
                  (mask_Y-N_pix/2+0.0642*N_pix)**2)
    mask3_radius = 0.026  # m
    mask3 = np.copy(r_3)*0.0
    idx3_1 = np.where(r_3 >= mask3_radius / (1./128.))
...
def make_beam_image_30arcsec():
    ...
    r_2 = np.sqrt((mask_X-N_pix/2-0.0874*N_pix)**2 +
                  (mask_Y-N_pix/2-0.0882*N_pix)**2)
...
def make_beam_image_40arcsec():
    ...
    r_2 = np.sqrt((mask_X-N_pix/2-0.1025*N_pix)**2 +
                  (mask_Y-N_pix/2-0.1034*N_pix)**2)
    mask2_radius = 0.253  # m
    mask2 = np.copy(r_2)*0.0
    idx2_1 = np.where(r_2 >= mask2_radius / (1./128.))
    idx2_2 = np.where(r_2 < mask2_radius / (1./128.))
...
def make_phase_screen():
    ...
    for i in range(len(EL)):
        ...
        for j in range(len(t_alt)):
            phi = aotools.turbulence.infinitephasescreen.PhaseScreenVonKarman(
                N_pix, pixel_scale, r0[i][j], L0)
...
def gauss2d(pos, A, x0, y0, w, q, pa):
    x = pos[0]
    y = pos[1]
    xc = (x - x0) * np.cos(np.deg2rad(-pa)) - \
        (y - y0) * np.sin(np.deg2rad(-pa))
    yc = (x - x0) * np.sin(np.deg2rad(-pa)) + \
        (y - y0) * np.cos(np.deg2rad(-pa))
...
for j in phscrn_el_arr:
    ...
    # phase maps in radian (it should be same pix scale as the gauss image)
    phscrn_el = j
    ...
    for i_EL in range(3):
        if i_EL == 0:
            ...
            # Optical path difference in m
            opd = phscrn_el[i_EL][0] * (wl_laser*1.0e-9/(2.0*math.pi))
            input_wave = poppy.ArrayOpticalElement(
                transmission=start_image, opd=opd*u.m, name='input wavefront', pixelscale=pixel_scale*u.m/u.pixel)
            # input_wave.display(what='both')

            # Setup initial condition for the fresnel propagation
            wf = poppy.FresnelWavefront(
                0.5*u.m, wavelength=wl_laser*1.0e-9, npix=N_pix, oversample=4)
            wf *= input_wave

            # 2D gaussian fit
            Xp, Yp = np.meshgrid(np.arange(0, N_pix*4, 1),
                                 np.arange(0, N_pix*4, 1))
            popt, pcov = opt.curve_fit(
                gauss2d, (Xp, Yp), wf.intensity.ravel(), p0=initial_guess)
            ...
            for i in range(len(t_alt)-1):
                ...
                # distance to the next turbulence plane in m
                dL = (t_alt[i+1] - t_alt[i]) * X[i_EL]
                ...
                # 2D gaussian fit
                Xp, Yp = np.meshgrid(
                    np.arange(0, N_pix*4, 1), np.arange(0, N_pix*4, 1))
                popt, pcov = opt.curve_fit(
                    gauss2d, (Xp, Yp), wf.intensity.ravel(), p0=initial_guess)
                gauss_fit = gauss2d((Xp, Yp), *popt)
                A_fit, x0_fit, y0_fit, w_fit, q_fit, pa_fit = popt
                w_arr.append(w_fit*pixel_scale)
                print("Waist size = %.4f" %
                      (w_fit*pixel_scale), t_alt[i+1], "i=", i)
                if i == 0:
                ...
                # add phase delay
                ph = poppy.ArrayOpticalElement(transmission=(start_image*0.0+1.0), opd=phscrn_el[i_EL][i+1] * (
                    wl_laser*1.0e-9/(2.0*math.pi)), name='', pixelscale=pixel_scale*u.m/u.pixel)
            ...
            Xp, Yp = np.meshgrid(np.arange(0, N_pix*4, 1),
                                 np.arange(0, N_pix*4, 1))
            popt, pcov = opt.curve_fit(
                gauss2d, (Xp, Yp), wf.intensity.ravel(), p0=initial_guess, maxfev=10000)
            gauss_fit = gauss2d((Xp, Yp), *popt)
            A_fit, x0_fit, y0_fit, w_fit, q_fit, pa_fit = popt
...
w_arr_average_EL30 = [np.mean(np.abs(w_05km_EL30)), np.mean(np.abs(w_1km_EL30)), np.mean(np.abs(w_2km_EL30)), np.mean(
    np.abs(w_4km_EL30)), np.mean(np.abs(w_8km_EL30)), np.mean(np.abs(w_16km_EL30)), np.mean(np.abs(w_90km_EL30))]
print(w_arr_average_EL30, "w_arr_average_EL30")
error_arr_average_EL30 = [np.std(w_05km_EL30)/len(w_05km_EL30), np.std(w_1km_EL30)/len(w_1km_EL30), np.std(w_2km_EL30)/len(w_2km_EL30), np.std(
    w_4km_EL30)/len(w_4km_EL30), np.std(w_8km_EL30)/len(w_8km_EL30), np.std(w_16km_EL30)/len(w_16km_EL30), np.std(w_90km_EL30)/len(w_90km_EL30)]
...
w_arr_average_EL60 = [np.mean(np.abs(w_05km_EL60)), np.mean(np.abs(w_1km_EL60)), np.mean(np.abs(w_2km_EL60)), np.mean(
    np.abs(w_4km_EL60)), np.mean(np.abs(w_8km_EL60)), np.mean(np.abs(w_16km_EL60)), np.mean(np.abs(w_90km_EL60))]
print(w_arr_average_EL60, "w_arr_average_EL60")
error_arr_average_EL60 = [np.std(w_05km_EL60)/len(w_05km_EL60), np.std(w_1km_EL60)/len(w_1km_EL60), np.std(w_2km_EL60)/len(w_2km_EL60), np.std(
    w_4km_EL60)/len(w_4km_EL60), np.std(w_8km_EL60)/len(w_8km_EL60), np.std(w_16km_EL60)/len(w_16km_EL60), np.std(w_90km_EL60)/len(w_90km_EL60)]
...
w_arr_average_EL90 = [np.mean(np.abs(w_05km_EL90)), np.mean(np.abs(w_1km_EL90)), np.mean(np.abs(w_2km_EL90)), np.mean(
    np.abs(w_4km_EL90)), np.mean(np.abs(w_8km_EL90)), np.mean(np.abs(w_16km_EL90)), np.mean(np.abs(w_90km_EL90))]
print(w_arr_average_EL90, "w_arr_average_EL90")
error_arr_average_EL90 = [np.std(w_05km_EL90)/len(w_05km_EL90), np.std(w_1km_EL90)/len(w_1km_EL90), np.std(w_2km_EL90)/len(w_2km_EL90), np.std(
    w_4km_EL90)/len(w_4km_EL90), np.std(w_8km_EL90)/len(w_8km_EL90), np.std(w_16km_EL90)/len(w_16km_EL90), np.std(w_90km_EL90)/len(w_90km_EL90)]
...
ax.errorbar(L_arr_average_EL30, w_arr_average_EL30, yerr=error_arr_average_EL30,
            fmt="*", capsize=3, color="blue", ecolor="blue")

L_arr_average_EL60 = np.array([0.5, 1., 2., 4., 8., 16., 90.])*X[1]  # km
print(L_arr_average_EL60, "L_arr_average_EL60")
ax.errorbar(L_arr_average_EL60, w_arr_average_EL60, yerr=error_arr_average_EL60,
            fmt="*", capsize=3, color="green", ecolor="green")

L_arr_average_EL90 = np.array([0.5, 1., 2., 4., 8., 16., 90.])*X[2]  # km
print(L_arr_average_EL90, "L_arr_average_EL90")
ax.errorbar(L_arr_average_EL90, w_arr_average_EL90,
            yerr=error_arr_average_EL90, fmt="*", capsize=3, color="red", ecolor="red")


z = np.arange(0, 200.0e+3, 100)
# Gaussian beam theoretical diffraction
wz = w0 * np.sqrt(1.0+(lam*z/(math.pi*w0**2))**2)
ax.plot(z/1000.0, wz, '--', c='black', label='Diffraction')  # 'Diffraction'

for i in range(3):
    # Seeing FWHM in arcsec
    fwhm_seeing = 1.082 * (lam/r0_total[i]) * (180.0/math.pi) * 3600.0
    ws = (fwhm_seeing / math.sqrt(2.0*math.log(2.0))) * \
        (math.pi/180.0)*(1.0/3600.0)*z
    ...
    fwhm_rc = np.sqrt((2.87)*w0**(-1.0/3.0)*np.sum(cn2 *
                      X[i]*(1.0-t_alt*X[i]/L)**2)) * 2.355 * (180.0/math.pi) * 3600.0
    fwhm_mod2 = math.sqrt(fwhm_seeing**2 - (fwhm_rc)**2)

    wsr2 = (fwhm_mod2 / math.sqrt(2.0*math.log(2.0))) * \
        (math.pi/180.0)*(1.0/3600.0)*z
    wsrz2 = np.sqrt(wz**2 + wsr2**2)
    if i == 0:
        ax.plot(z/1000.0, wsz, '-', c='blue')  # 'Diff.+Seeing(empirical)'
        # 'Diff+Seeing(Gaussian jitter removed)'
        ax.plot(z/1000.0, wsrz2, '-.', c='blue',
                label='Diff+Seeing(Gaussian jitter removed) (EL30)')
    elif i == 1:
        ax.plot(z/1000.0, wsz, '-', c='green')
        ax.plot(z/1000.0, wsrz2, '-.', c='green',
                label='Diff+Seeing(Gaussian jitter removed) (EL60)')
    elif i == 2:
        ax.plot(z/1000.0, wsz, '-', c='red')
        ax.plot(z/1000.0, wsrz2, '-.', c='red',
                label='Diff+Seeing(Gaussian jitter removed) (EL90)')
